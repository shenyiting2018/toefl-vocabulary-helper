		...
		if ($("#kill-3000-table").length) {
			var table = $("#kill-3000-table").DataTable({
				...
				fnDrawCallback: function(e) {
					hiddenButtonHandler();
					detailsControlButtonHandler(this, 'kill-3000-table');
					proficiencyHandler();
				},
				columnDefs: [
...
function init_AdditionalDataTables() {
	console.log('run_datatables');

	if( typeof ($.fn.DataTable) === 'undefined'){ return; }
	console.log('init_DataTables');

	var handleAdditionalDataTableButtons = function() {
		if ($("#additional-word-table").length) {
			var table = $("#additional-word-table").DataTable({
				pagingType: "input",
				pageLength: 50,
				dom: "Blfrtip",
				fnDrawCallback: function(e) {
					hiddenButtonHandler();
					detailsControlButtonHandler(this, 'additional-word-table');
					proficiencyHandler();
				},
				columnDefs: [
					{ "visible": false, "targets": 1 }
	...
	TableManageButtons = function() {
		...
		return {
			init: function() {
				handleAdditionalDataTableButtons();
			}
		};
	}();
...
var detailsControlButtonHandler = function(table, tableName) {
	$('.details-control').off('click');
	$('#' + tableName + ' tbody').off('click', 'button.details-control');
	$('#' + tableName + ' tbody').on('click', 'button.details-control', function () {
...
var buildProficiencyDropdown = function(currentProficiency, wordId, categoryName) {
	...
	+ '<li><button class="btn btn-primary column-button prof-button" value="4" word-id=' + wordId + ' category-name=' + categoryName + ' href="#">4</button></li>'
	+ '<li class="divider"></li>'
	+ '<li><button class="btn btn-success column-button prof-button" value="3"  word-id=' + wordId + ' category-name=' + categoryName + ' href="#">3</button></li>'
	+ '<li><button class="btn btn-info column-button prof-button" value="2" word-id=' + wordId + ' category-name=' + categoryName + ' href="#">2</button></li>'
	+ '<li><button class="btn btn-warning column-button prof-button" value="1" word-id=' + wordId + ' category-name=' + categoryName + ' href="#">1</button></li>'
	+ '<li class="divider"></li>'
	+ '<li><button class="btn btn-danger column-button prof-button" value="0" word-id=' + wordId + ' category-name=' + categoryName + ' href="#">0</button></li>'
	+ '</div>'
	+ '</div>';
}
...
$(document).ready(function() {
	...
	init_AdditionalDataTables(hiddenButtonHandler);
	var drawWordsTablehandler = function(categoryWords) {
		...
		for (word of categoryWords) {
			...
			wordProficiency = buildProficiencyDropdown(word.proficiency, word.id, word.category.categoryName);
	...
	var drawAdditionalWordsTablehandler = function(categoryWords) {
		dt = $('#additional-word-table').DataTable();
		dt.clear();
		var number = 1;
		for (word of categoryWords) {
			wordMeanings = '<span class="table-word-meanings-column" value=' + word.meanings + '>'
			...
			wordProficiency = buildProficiencyDropdown(word.proficiency, word.id, word.category.categoryName);
			node = dt.row.add([numberButton, word.id, '<b>' + word.wordString + '</b>', wordMeanings, word.category.categoryName, wordProficiency]);
			number += 1;
		}
	...
	var addWord = function(word, meaning, category, loadingDom, handler) {
		if (!!loadingDom) {
			loadingDom.loading();
		}
		...
		$.post('/toefl/vocabularies/add-user-word', param, function(data, status) {
			if (status === 'success') {
				if (data.status === 'success') {
					alert('Word added successfully!');
					getGREAdditionalWords(drawAdditionalWordsTablehandler, $('#additional-word-table'));
				} else {
					console.log('error');
				}
			} else {
	...
	additionalWords = getGREAdditionalWords(drawAdditionalWordsTablehandler, $('#additional-word-table'));

	$('#add-word-button').off('click');
	$('#add-word-button').on('click', function(event) {
		event.preventDefault();
		var word = $('#word-string-input').val();
		var meaning = $('#word-meaning-input').val();
		var category = $('#word-category-input option:selected').val();

		if (!checkAddedWordValid(word, meaning, category)) {
			alert('Invalid word, please make sure you set each field!');
			return;
		}
		...
		addWord(word, meaning, category, $('#additional-word-table'));
	});
	//Additional Words
});
